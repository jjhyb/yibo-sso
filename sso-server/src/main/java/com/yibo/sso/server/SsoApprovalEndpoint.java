...
public class SsoApprovalEndpoint {
    ...
    private static String DENIAL = "<form id='denialForm' name='denialForm' action='${path}/oauth/authorize' method='post'>" +
            "<input name='user_oauth_approval' value='false' type='hidden'/>%csrf%<label><input name='deny' value='Deny' type='submit'/></label></form>";

    /**
     * TEMPLATE为用户登录后的授权页面，让用户登录后直接进入页面，不授权体验更好，
     * 自动授权思路：改用户登录后的手动授权为自动授权，增强用户体验
     */
    /*private static String TEMPLATE = "<html><body><div style='display:none;'><h1>OAuth Approval</h1>" +
            ...
            "<input name='authorize' value='Authorize' type='submit'/></label></form>%denial%</div><script>document.getElementById('confirmationForm').submit()</script></body></html>";*/
    ...
    private static String SCOPE = "<li><div class='form-group'>%scope%: <input type='radio' name='%key%' value='true'%approved%>" +
